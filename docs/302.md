...
这些操作是异步执行的，可能要花一些时间才能执行. 对于给定的容器存储库，您最多可以每小时运行一次. 此操作不会删除 Blob. 为了删除它们并回收磁盘空间，请[运行垃圾回收](https://s0docs0gitlab0com.icopy.site/omnibus/maintenance/README.html) .
