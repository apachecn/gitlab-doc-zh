...
*   并非所有执行程序都[受支持](https://s0docs0gitlab0com.icopy.site/runner/executors/) .
*   默认情况下，仅为成功的作业收集作业工件.
...
**注意：** `GIT_STRATEGY`器不支持[GIT_STRATEGY](https://s0docs0gitlab0com.icopy.site/runner/executors/kubernetes.html) ，但将来可能会支持. 有关更新，请参阅[带有 Kubernetes 执行器功能建议](https://gitlab.com/gitlab-org/gitlab-runner/-/issues/3847)的[支持 Git 策略](https://gitlab.com/gitlab-org/gitlab-runner/-/issues/3847) .
...
| **EXECUTOR_JOB_SECTION_ATTEMPTS** | [从 GitLab 12.10 开始](https://gitlab.com/gitlab-org/gitlab-runner/-/issues/4450) ，出现[`No Such Container`](https://gitlab.com/gitlab-org/gitlab-runner/-/issues/4450)错误（仅限[Docker 执行器](https://s0docs0gitlab0com.icopy.site/runner/executors/docker.html) ）后，尝试在作业中运行部分的次数. |
...
**注意：**仅当在[Runner 的配置中](https://s0docs0gitlab0com.icopy.site/runner/configuration/advanced-configuration.html)启用`custom_build_dir`时，才可以使用此`custom_build_dir` . 这是默认配置`docker`和`kubernetes`执行.
...
`GIT_CLONE_PATH`必须始终在`$CI_BUILDS_DIR` . `$CI_BUILDS_DIR`设置的目录取决于执行程序和[runners.builds_dir](https://s0docs0gitlab0com.icopy.site/runner/configuration/advanced-configuration.html)设置的配置.
